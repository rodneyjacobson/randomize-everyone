...
class UserDAOImpl @Inject() (protected val dbConfigProvider: DatabaseConfigProvider) extends UserDAO {
  ...
  def find(loginInfo: LoginInfo): Future[Option[User]] = {
    ...
    db.run(userQuery.result.headOption).map { dbUserOption =>
      dbUserOption.map {
        user => User(UUID.fromString(user.userID), loginInfo, user.firstName, user.lastName, user.netID, user.email, user.avatarUrl, user.activated)
      }
    }
  }
  ...
  def find(): Future[Seq[User]] = {
    ...
    db.run(userQuery.result).map { resultOption =>
      resultOption.map {
        case (user, loginInfo) => User(
          UUID.fromString(user.userID),
          LoginInfo(loginInfo.providerId, loginInfo.providerKey),
